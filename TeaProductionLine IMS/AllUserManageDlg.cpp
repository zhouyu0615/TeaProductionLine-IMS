...
void CAllUserManageDlg::OnBnClickedBt1Adduser()
{
	...
	m_pDataProvider->ReadLoginUserFormDatabase();				//重新读一次数据库，因为容器里的数据和数据库里的创建时间不能完全同步///
...
void CAllUserManageDlg::MyOnPint()
{
	...
	m_list_UserInfo.InsertColumn(1, _T("序号"), LVCFMT_CENTER, rect1.Width() / 35 * 2, -1);
...
void CAllUserManageDlg::OnNMRClickLi1Allusermanagedlg(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	{
		int nResult;
		nResult = MessageBox(_T("该操作会使删除后的数据无法恢复，是否删除？"), _T("警告"), MB_ICONEXCLAMATION | MB_YESNO);//警告//
		if (nResult == IDYES)
		{
			long ManagerId1 = CLoginDlg::LoginUser_CurrentId;		//当前登录为管理员，将管理员ID取出，禁止管理员删除自己的信息///
			if (ManagerId1 == m_pDataProvider->m_vectLoginUser[index].m_Id)		//若当前删除的用户ID为管理员，则提示不能删除///
			{
				AfxMessageBox(_T("对不起,您不能删除自己的信息!"));
				return;
			}
	...
	if (nItem1 == ID_MODIFY)
	{
		...
		dlgUserPop.DoModal();
		m_pDataProvider->ReadLoginUserFormDatabase();  //重新读一次数据库，因为容器里的数据和数据库里的修改时间不能完全同步///
		MyOnPint();
	}
}
...
void CAllUserManageDlg::OnBnClickedBt2Clearall()
{
	// TODO:  在此添加控件通知处理程序代码
	int nResult;
	nResult = MessageBox(_T("该操作会使除您之外的所有用户信息删除,清空后的数据无法恢复，是否清空？"),
		_T("警告"), MB_ICONEXCLAMATION | MB_YESNO);
	if (nResult == IDYES)
	{
		long ManagerId2 = CLoginDlg::LoginUser_CurrentId;		//当前登录为管理员，将管理员ID取出，禁止管理员删除自己的信息///
